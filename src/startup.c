...
void activate (GtkApplication* app, gpointer user_data)
{
    ...
    // initialize context_list add to ezeedo wrapper structure
    category_container* context_list = calloc(1, sizeof(category_container));
    ezeedo->context_list = context_list;

    // initialize project_list add to ezeedo wrapper structure
    ...
    width        = g_settings_get_int    (settings, "main-window-width");
    height       = g_settings_get_int    (settings, "main-window-height");
    x            = g_settings_get_int    (settings, "main-window-position-y");
    y            = g_settings_get_int    (settings, "main-window-position-y");
    ...
    gtk_window_set_title        (GTK_WINDOW (window), "Ezeedo");
    gtk_window_set_icon_name    (GTK_WINDOW (window), EZEEDOICON);
    ...
    todolist_box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
    ...
    gtk_widget_set_size_request (todolist_box_left, 150, -1);


    // create scrolled categories container and box
    todo_categories_container_scrolled = gtk_scrolled_window_new (NULL, NULL);
    ...
    todolist_container_scrolled = gtk_scrolled_window_new(NULL, NULL);

    // create task entry
    task_entry = gtk_entry_new ();
    gtk_widget_set_margin_start       (GTK_WIDGET(task_entry), 10);
    gtk_widget_set_margin_end         (GTK_WIDGET(task_entry), 10);
    gtk_widget_set_margin_top         (GTK_WIDGET(task_entry), 10);
    gtk_widget_set_margin_bottom      (GTK_WIDGET(task_entry), 10);
    // gtk_entry_set_icon_from_icon_name (GTK_ENTRY(task_entry), GTK_ENTRY_ICON_PRIMARY, EZEEDOICON);
    gtk_entry_set_placeholder_text    (GTK_ENTRY(task_entry), "Enter new task");
    gtk_entry_set_max_length          (GTK_ENTRY(task_entry), TASKLENGTH);
    gtk_widget_grab_focus(task_entry);
   
    
    // create main box for donelist
    donelist_box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);

    // create scrolled donelist container
    donelist_container_scrolled = gtk_scrolled_window_new (NULL, NULL);
    gtk_widget_set_vexpand(donelist_container_scrolled, TRUE);

    // create archive button
    archive_button = gtk_button_new_with_label("Archive all done tasks");
    gtk_widget_set_margin_start  (GTK_WIDGET (archive_button), 10);
    gtk_widget_set_margin_end    (GTK_WIDGET (archive_button), 10);
    gtk_widget_set_margin_top    (GTK_WIDGET (archive_button), 10);
    gtk_widget_set_margin_bottom (GTK_WIDGET (archive_button), 10);

    // create about action and connect about action signal
    about_action = g_simple_action_new ("about", NULL); 
    g_action_map_add_action (G_ACTION_MAP (window), G_ACTION (about_action));
    g_signal_connect (about_action, "activate", G_CALLBACK (show_about_window), window);
    ...
    gtk_container_add    (GTK_CONTAINER(todo_categories_container_scrolled), todo_categories_box);
    gtk_container_add    (GTK_CONTAINER(todolist_box_left), todo_categories_container_scrolled);
    gtk_container_add    (GTK_CONTAINER(todolist_box_right), todolist_container_scrolled);
    gtk_paned_pack1      (GTK_PANED(todolist_box_paned), todolist_box_left, FALSE, TRUE);
    gtk_paned_pack2      (GTK_PANED(todolist_box_paned), todolist_box_right, TRUE, TRUE);
    gtk_container_add    (GTK_CONTAINER(todolist_box), todolist_box_paned);
    gtk_container_add    (GTK_CONTAINER(todolist_box), task_entry);

    gtk_container_add    (GTK_CONTAINER(donelist_box), donelist_container_scrolled);
    gtk_container_add    (GTK_CONTAINER(donelist_box), archive_button);

    gtk_stack_add_titled (GTK_STACK(stack), todolist_box, "To do", "To do");
    gtk_stack_add_titled (GTK_STACK(stack), donelist_box, "Done", "Done");
